...
def make_sequences(
    ...
    y  : next-day *scaled* target, shape (n_samples,)
    close_t : Close price at t (to reconstruct price_{t+1})
    dates   : DatetimeIndex for t+1 (for plotting)
    """
    X, y, close_t, dates = [], [], [], []
...
class PriceLSTM(torch.nn.Module):
    def __init__(self, n_features: int, hidden: int = 16, dropout: float = 0.3):
        super().__init__()
        ...
        self.fc1 = torch.nn.Linear(hidden, 8)
...
def train_lstm_same_info(
    ...
    # Feature engineering identical to ARIMAX info set
    df_feat = df.copy()
    ...
    X_train, X_test = X[: split_idx - (seq_len - 1)], X[split_idx - (seq_len - 1) :]
    y_train, y_test = y_scaled[: split_idx - (seq_len - 1)], y_scaled[split_idx - (seq_len - 1) :]
    close_t_train, close_t_test = close_t[: split_idx - (seq_len - 1)], close_t[split_idx - (seq_len - 1) :]
    dates_test = dates[split_idx - (seq_len - 1) :]
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_loss < best_val - 1e-5:        # tiny margin
            best_val = val_loss
            patience_cnt = 0
        ...
        if epoch % 20 == 0 or epoch == 1:
            print(f"Epoch {epoch:3d} | val MSE={val_loss:.6f} | lr={scheduler.get_last_lr()[0]:.1e}")
