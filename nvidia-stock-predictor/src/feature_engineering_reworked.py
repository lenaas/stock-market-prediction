...
def add_lag_features(
    ...
    if debug:
        lag_cols = df.filter(regex="_l(1|20)$").columns
...
def prepare_merged_data(
    ...
    # fill early NaNs introduced by lags to retain as much data as possible
    df = df.fillna(method="bfill").dropna().sort_index()
    if debug:
