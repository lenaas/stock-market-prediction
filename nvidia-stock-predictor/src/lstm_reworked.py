...
class PriceLSTM(torch.nn.Module):
    def __init__(self, n_features: int, hidden: int = 16, dropout: float = 0.2):
        super().__init__()
...
def train_lstm_same_info(
    ...
    test_split = int(len(df_feat) * 0.8)
    val_split = int(test_split * 0.9)

    scaler_x = StandardScaler().fit(df_feat[feature_cols].iloc[:val_split])
    scaler_y = StandardScaler().fit(df_feat[["log_return"]].iloc[:val_split])
    ...
    train_adj = train_end - (seq_len - 1)
    val_adj = val_end - (seq_len - 1)

    X_train = X[:train_adj]
    y_train = y_scaled[:train_adj]
    X_val = X[train_adj:val_adj]
    y_val = y_scaled[train_adj:val_adj]
    X_test = X[val_adj:]
    y_test = y_scaled[val_adj:]

    close_t_train = close_t[:train_adj]
    close_t_val = close_t[train_adj:val_adj]
    close_t_test = close_t[val_adj:]
    dates_test = dates[val_adj:]
